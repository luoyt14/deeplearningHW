...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            msg = '  Training iter %d, avg loss %.4f, avg acc %.4f' % (iter_counter, np.mean(loss_list), np.mean(acc_list))
            loss_list = []
            acc_list = []
            LOG_INFO(msg)
...
def get_feature_map(model, inputs, labels):
    num = 0
    for input, label in data_iterator(inputs, labels, 1, shuffle=False):
        if label[0] == num:
            output = model.forward_one_layer(input)
            for i in range(output.shape[1]):
                plt.subplot(1, output.shape[1], i + 1)
                output_img = output[0, i, :, :]
